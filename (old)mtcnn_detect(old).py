...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  2 21:56:24 2017
...
def detect(path):
    ...
    # img_matlab[:,:,0],img_matlab[:,:,2] =img_matlab[:,:,2], img_matlab[:,:,0]
...
def getpaths(dir):
    ls = os.listdir(dir)
    paths = [os.path.join(dir,f) for f in ls if os.path.splitext(f)[1] == '.jpg' or os.path.splitext(f)[1] == '.png']
    return paths

if __name__ == "__main__":
    # detect(sys.argv[1])
    imagedir = os.path.join(os.getcwd(),'smoking')
